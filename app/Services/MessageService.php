...
class MessageService
{
    ...
    public function calculateMessageCost(string $content, int $recipientCount)
    {
        ...
        $costPerSegment = config('services.messaging.cost_per_segment', 5.99);
    ...
    public function sendDirectMessage(User $user, array $data)
    {
        try {
            ...
            if (isset($data['webhook_url'])) {
                // Use user-provided webhook URL
                $callbackUrl = $data['webhook_url'];
            } else {
                // Use our internal webhook endpoint
                $callbackUrl = config('app.url') . '/api/webhooks/hollatags/delivery';

                // Add reference if provided
                if (isset($data['reference'])) {
                    $callbackUrl .= '/' . $data['reference'];
                }
            }
            ...
            if (count($recipients) == 1) {
                // Send single message
                $result = $this->hollaTagsService->sendSingleMessage($message, $recipients[0], $callbackUrl);
            } else {
                // Send bulk message
                $result = $this->hollaTagsService->sendBulkMessage($message, $recipients, $callbackUrl);
            }
